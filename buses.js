(() => {
  ...
  function ensureView() {
    ...
    nav.innerHTML = '<button id="accountsTab" class="secondary" type="button">Accounts</button><button id="busesTab" class="primary" type="button">Buses</button>';
    main.prepend(nav);
    const accounts = document.querySelector('.head');
    const panel = accounts?.nextElementSibling;
    if (accounts) accounts.dataset.accountsView = '1';
  ...
  function toggle(show) {
    const main = document.querySelector('.admin-main'); const bus = $('busAdmin'); if (!main || !bus) return;
    [...main.children].forEach(el => { if (el !== bus && !el.classList.contains('admin-tabs')) el.hidden = show; });
    bus.hidden = !show;
    $('accountsTab').className = show ? 'secondary' : 'primary'; $('busesTab').className = show ? 'primary' : 'secondary';
    if (show) loadBuses();
  }
  function render() {
  ...
  const boot = setInterval(() => { if (window.supabase && window.sb && $('dashboard')) { clearInterval(boot); ensureView(); } }, 100);
})();
